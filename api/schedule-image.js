...
const { getSchedule: getScheduleFromDb } = require('./_lib/db');

const JSON_DAY_MAP = {
  monday: 'Понеділок', tuesday: 'Вівторок', wednesday: 'Середа',
  thursday: 'Четвер', friday: "П'ятниця", saturday: 'Субота', sunday: 'Неділя'
};
const JSON_LESSON_TIMES = {
  1: { start: '08:30', end: '09:50' }, 2: { start: '10:00', end: '11:20' },
  3: { start: '11:50', end: '13:10' }, 4: { start: '13:20', end: '14:40' },
  5: { start: '14:50', end: '16:10' }, 6: { start: '16:20', end: '17:40' }
};

function jsonNormalizeGroup(g) {
  if (!g) return '';
...
function jsonGetWeekKey(groupData) {
  const types = Object.keys(groupData).filter(t => t !== 'ПІДВІСКА');
  const hasChis = types.includes('ЧИСЕЛЬНИК');
  const hasZnam = types.includes('ЗНАМЕННИК');
...
async function handleJsonSchedule(req, res) {
  ...
  if (!group || !day) return res.status(400).json({ error: 'group and day are required' });

  const dayName = JSON_DAY_MAP[day.toLowerCase()];
  if (!dayName) return res.status(400).json({ error: 'invalid day. Use: monday, tuesday, ...' });

  try {
    const scheduleData = await getScheduleFromDb();
    const ng = jsonNormalizeGroup(group);
    let groupName = null;
    for (const key of Object.keys(scheduleData)) {
      if (key === '_settings') continue;
      if (jsonNormalizeGroup(key).toLowerCase() === ng.toLowerCase() || key.toLowerCase() === group.toLowerCase()) {
        groupName = key;
        break;
      }
    }
    if (!groupName) return res.status(200).json({ group, day, lessons: [] });

    const groupData = scheduleData[groupName];
    const types = Object.keys(groupData).filter(t => t !== 'ПІДВІСКА');
    if (types.length === 0) return res.status(200).json({ group, day, lessons: [] });

    const weekKey = jsonGetWeekKey(groupData);
    let weekData = groupData[weekKey];
    if (!weekData || typeof weekData !== 'object' || Array.isArray(weekData)) {
      weekData = groupData[types[0]];
    }
    ...
    const today = new Date();
    const currentDow = today.getDay() || 7;
    const dayIdx = Object.values(JSON_DAY_MAP).indexOf(dayName) + 1;
    const offset = dayIdx - currentDow;
    const d = new Date(today);
    d.setDate(today.getDate() + offset);
    const dateStr = String(d.getDate()).padStart(2, '0') + '.' + String(d.getMonth() + 1).padStart(2, '0');
    ...
    if (pairs.length === 0) return res.status(200).json({ group, day, lessons: [] });
    pairs.sort((a, b) => parseInt(a.number) - parseInt(b.number));

    const lessons = pairs.map(p => {
      const times = JSON_LESSON_TIMES[p.number] || { start: '', end: '' };
      return {
        number: p.number, name: p.subject || '', teacher: p.teacher || '',
        room: p.room || '', time_start: times.start, time_end: times.end,
        isSubstitution: p.isSubstitution || false, substitutionType: p.substitutionType || null
      };
    });

    return res.status(200).json({ group: groupName, day, date: dateStr, lessons });
  } catch (err) {
    console.error('schedule JSON error:', err);
    return res.status(500).json({ error: 'Internal server error' });
  }
}

module.exports = async function handler(req, res) {
  if (req.query.format === 'json') return handleJsonSchedule(req, res);
